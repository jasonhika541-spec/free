...
# Auto-detect available iOS SDK
AVAILABLE_SDKS := $(shell ls $(THEOS)/sdks/ 2>/dev/null | grep "iPhoneOS[0-9].*\.sdk" | head -1)
ifneq ($(AVAILABLE_SDKS),)
    SDK_VERSION := $(shell echo "$(AVAILABLE_SDKS)" | sed 's/iPhoneOS\([0-9]*\.[0-9]*\)\.sdk/\1/')
    ifneq ($(SDK_VERSION),)
        TARGET := iphone:clang:$(SDK_VERSION)
    else
        TARGET := iphone:clang:latest
    endif
else
    # Fallback to latest without version
    TARGET := iphone:clang
endif
